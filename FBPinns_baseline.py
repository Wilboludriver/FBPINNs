...
class FBPinns:
    ...
    def get_ovl_residual(self, input_int,model1,model2,sub1,sub2,uni1,uni2):
        ...
        u_1 = unnormalize( model1(normalize(input_int,subdomain=sub1)))*window(input_int,uni1,sigma=0.01)
        u_2 = unnormalize( model2(normalize(input_int,subdomain=sub2)))*window(input_int,uni2,sigma=0.01)
        w_max =  torch.Tensor(self.w_list).max()
    ...
    def plot_window_funcitons(self,sig,title):
        ...
        for i in range(self.n_sub):
            ...
            plt.scatter(ovl_set.detach(), y_wind.detach(), s=1)
...
if __name__ == '__main__':
    work_dir = '/home/xdjf/dlsc/1d/fbpinn_baseline/'
    train = False
    n_int = 300 * 200
    n_layers = 2
    n_neurons = 16
    ovl=0.3
    fbpinn = FBPinns(n_int_=n_int,w_list=[1,15],n_layers=n_layers,n_neurons=n_neurons,n_subdomain=30,overlap_len=ovl)
    ...
    if train == True:
        ...
        fbpinn.plot_window_funcitons(sig=0.001,title='(c) Window function sigma =0.001')
